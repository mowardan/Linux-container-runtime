...
### 3.2 Namespaces Subsystem (`src/namespaces/`) - *Planned*
- **PID Namespace (`CLONE_NEWPID`)**: Isolates the process ID space. The container's primary process becomes PID 1.
- **UTS Namespace (`CLONE_NEWUTS`)**: Isolates hostname and NIS domain name.
- **Mount Namespace (`CLONE_NEWNS`)**: Isolates filesystem mount tables and mount propagation trees.
